...
1. 机器人基本参数 (部署到新机器人时必须修改):
- robot_mass: 机器人总质量(kg), 用于动力学计算和力控
- a1_trunk_inertia: 机身惯性矩阵, 用于动力学计算
- default_foot_pos: 机器人四条腿的默认位置(m), 以机身坐标系为参考系
  FL(左前), FR(右前), RL(左后), RR(右后)的x,y,z坐标
  x正方向为前, y正方向为左, z正方向为上

2. 控制器增益参数 (需要根据实际控制效果调整):
- kp_foot/kd_foot: 足端位置控制PD参数
  kp越大响应越快但容易振荡,kd用于阻尼
- kp_linear/kd_linear: 机身线速度控制PD参数
  控制机身在x,y,z方向的运动
- kp_angular/kd_angular: 机身角速度控制PD参数
  控制机身绕x,y,z轴的转动
- q_weights/r_weights: MPC控制器的权重参数
  q_weights权衡状态跟踪误差,r_weights权衡控制量大小

3. 步态参数 (可根据需要调整):
- counter_per_gait: 一个完整步态周期的控制周期数
- counter_per_swing: 单腿摆动阶段的控制周期数
- gait_counter_speed: 各腿的步态速度

4. 其他重要参数:
- stance_leg_control_type: 支撑腿控制方式(0:QP, 1:MPC)
- use_terrain_adapt: 是否启用地形适应(0:否, 1:是)
- power_level: 电机功率等级(1-5),影响最大输出扭矩

部署步骤:
1. 首先修改机器人基本参数以匹配实际硬件
2. 从较小的PD增益开始调试,逐步提高到合适的值
3. 调整步态参数实现期望的运动效果
4. 根据实际需求开启地形适应等功能
*/
...
public:
    ...
    void resetFromROSParam(ros::NodeHandle &_nh)
    {
        ...
        kp_foot << a1_kp_foot_x, a1_kp_foot_x, a1_kp_foot_x, a1_kp_foot_x,
            a1_kp_foot_y, a1_kp_foot_y, a1_kp_foot_y, a1_kp_foot_y,
            a1_kp_foot_z, a1_kp_foot_z, a1_kp_foot_z, a1_kp_foot_z;
        kd_foot << a1_kd_foot_x, a1_kd_foot_x, a1_kd_foot_x, a1_kd_foot_x,
        ...
        gait_counter_speed
            << a1_gait_counter_speed_FL,
            a1_gait_counter_speed_FR, a1_gait_counter_speed_RL, a1_gait_counter_speed_RR;
